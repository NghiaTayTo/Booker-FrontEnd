...
import { faArrowDownLong, faEyeLowVision } from "@fortawesome/free-solid-svg-icons";
...
const ListProduct = ({ listBooks = [], keySearch, searchName }) => {
    ...
    const tableRows = currentProducts.map((book, index) => (
        <tr key={index} className={`box-shadow_row ${getTrangThaiRow(book.trang_thai_khoa, book.an_san_pham)}`}
            ...
            <td style={{ width: '100px', textAlign: 'center' }}>{book.da_ban ? book.da_ban : 0}</td>
